...
case "${1:-menu}" in
  ...
  menu)
    ...
    case "$choice" in
      ...
      2) run_cmd uv run jupyter lab ;;
      3) run_cmd uv run qfl-market-demo ;;
      4) run_cmd uv run qfl-demo-data ;;
      5) run_cmd uv run qfl-data ;;
      6) run_cmd uv run qfl-backtest ;;
      7) run_cmd uv run qfl-signal ;;
      8) run_cmd uv run pytest ;;
      0) exit 0 ;;
      *) echo "无效选择: $choice" ;;
    esac
    ;;
